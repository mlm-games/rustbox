...
pub fn rapier_plugin(app: &mut App) {
    app.add_plugins(RapierPhysicsPlugin::<NoUserData>::default())
        .add_systems(Update, (sync_drift_plates, apply_launch_impulses, sync_seal_open));
}
...
fn apply_launch_impulses(
    ...
    for (pad_tf, mut pad) in &mut pads {
        ...
        for (ptf, mut impulse) in &mut player {
            ...
            if flat.distance(pad_flat) < 0.8 && (ptf.translation.y - pad_tf.translation.y).abs() < 1.3 {
...
fn sync_seal_open(
    ...
    q: Query<(Entity, &Seal), Changed<Seal>>,
) {
    for (e, seal) in &q {
        if seal.open {
